...
Design priorities (highest first):
  1. Numerical fidelity to the mathematical formula.
  2. Independence from the device implementation (so this is a true
     external reference, not a circular check).
  3. Reproducibility and clarity.

We accept lower performance in exchange for these. Concretely:
  - All internal arithmetic is float64 (numpy default precision).
  - Force computation uses direct division /, not "multiply by inverse",
    which would add an extra ULP of error per pair.
  - Force accumulation uses Kahan compensated summation to keep the
    accumulator's error at O(eps) rather than O(N*eps).
  - The pair loop is explicit (i,j) rather than vectorized, so the
    accumulation order is the canonical one (j = 0, 1, ..., N-1)
    independent of any numpy implementation details (numpy's vectorized
    sum uses pairwise summation, which is an implementation choice we
    don't want sneaking into our reference).
  - sqrt is np.sqrt in f64, which on IEEE-754 systems is correctly
    rounded.
  - Only the final saved arrays are cast to float32, to match what the
    device exports.
...
def compute_accelerations(x, m):
    ...
    for i in range(N):
        ...
        for j in range(N):
            ...
            r      = np.sqrt(r2_eps)            # correctly-rounded f64
            r3     = r * r2_eps                  # r^3 (softened)

            # Direct division: f_scalar = G * m_j / r^3.
            # Note: NOT "multiply by 1/r^3", which would lose one ULP.
...
def run_leapfrog(bodies, n_steps, dt, energy_track=False):
    ...
    energy_hist = []
    if energy_track:
        # At step 0 the input velocity v_0 is the integer-step velocity.
